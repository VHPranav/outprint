...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b border-[#E5E5E5] last:border-b-0", className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 text-left font-sans text-sm sm:text-base font-medium text-neutral-900 transition-all hover:text-black [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
